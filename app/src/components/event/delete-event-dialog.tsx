...
export const DeleteEventDialog = ({ eventId }: { eventId: string }) => {
  ...
  return (
    <Dialog open={isDeleteDialogOpen} onOpenChange={handleDialogClose}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Warning: You are about to delete this event!</DialogTitle>
          <DialogDescription>
        ...
        <DialogFooter>
          <div className="flex w-full align-bottom justify-around pt-5">
            <Button size="sm" onClick={handleDialogClose}>No. Go back.</Button>
            <Button size="sm" variant="destructive" onClick={() => deleteEventById(eventId)}>Yes, I'm sure.</Button>
          </div>
        </DialogFooter>
      </DialogContent>
    </Dialog>
